...
mens_world_records_seconds <- mens_world_records_clean %>%
  mutate(Perf = case_when(!str_detect(Perf, ":") ~ Perf,
                          str_detect(Perf, ":{1}") ~ lubridate::period_to_seconds(lubridate::ms(Perf))),
                          str_detect(Perf, ":{2}") ~ lubridate::period_to_seconds(lubridate::hms(Perf)))
...
```{r}
world_record_100 <- readr::read_csv(file = here("Data/100mworldrecord.csv"), col_types = list(
  col_character(),
  col_character(),
  col_character(),
  col_character(),
...
```{r}
fourhundred_progression_one <- wikipedia_table_scrape(
  url = "https://en.wikipedia.org/wiki/Men%27s_400_metres_world_record_progression",
  xpath = '//*[@id="mw-content-text"]/div[1]/table[1]'
)
fourhundred_progression_two <- wikipedia_table_scrape(
  url = "https://en.wikipedia.org/wiki/Men%27s_400_metres_world_record_progression",
  xpath = '//*[@id="mw-content-text"]/div[1]/table[2]'
)
```
...
```{r}
fifteenhundred_progression <- wikipedia_table_scrape(
...
```{r}
threethousand_progression <- wikipedia_table_scrape(
...
```{r}
twomile <- wikipedia_table_scrape(
...
```{r}
fivethousand_progression <- wikipedia_table_scrape(
...
```{r}
tenthousand_progression <- wikipedia_table_scrape(
...
```{r}
marathon <- wikipedia_table_scrape(
...
final_df <- thirteentot_clean %>%
  mutate(
    Location = str_remove(Location, "\\[2\\]"),
  ...
  mutate(
    Location = ifelse(Athlete == "Daniel Komen" & Date == "1996-07-14",
